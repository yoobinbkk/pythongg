...
block = soup.select('.tabcont')
for i in block:
    구청이름 += [i.select_one("strong").text]
    구청주소 += [i.select_one("li:nth-child(1)").text]
    구청전화번호 += [i.select_one("li:nth-child(2)").text]
    구청홈페이지 += [i.select_one("li:nth-child(3)").text]

for i in range(len(구청이름)):
    print(f"구청이름 : {구청이름[i]}")
    print(f"구청주소 : {구청주소[i]}")
    print(f"구청전화번호 : {구청전화번호[i]}")
    print(f"구청홈페이지 : {구청홈페이지[i]}")
    print()
